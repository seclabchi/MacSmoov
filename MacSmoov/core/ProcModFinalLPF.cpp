...
ProcModFinalLPF::ProcModFinalLPF(const string& _name, uint32_t _f_samp, uint8_t _n_channels, uint32_t _n_samps) : ProcessorModule (_name, _f_samp, _n_channels, _n_samps) {

    filt_L = new tonekids::dsp::FilterLR4(_f_samp, 15000.0, tonekids::dsp::BUTTERWORTH_TYPE::kLoPass, _n_samps);
    filt_R = new tonekids::dsp::FilterLR4(_f_samp, 15000.0, tonekids::dsp::BUTTERWORTH_TYPE::kLoPass, _n_samps);
...
ProcModFinalLPF::~ProcModFinalLPF() {
    delete filt_L;
    delete filt_R;
}

bool ProcModFinalLPF::init_impl(CoreConfig* cfg, ProcessorModule* prev_mod, ChannelMap* _channel_map) {
    if(nullptr != cfg) {
    }
