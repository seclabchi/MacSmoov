...
bool Butterworth::coefficients(BUTTERWORTH_TYPE filter, const double fs, const double freq1_cutoff,
    ...
    std::cout << "[Butterworth Filter Design] prewarped f1 = " << f1 << std::endl;
    std::cout << "[Butterworth Filter Design] prewarped f2 = " << f2 << std::endl;
#endif
    ...
    int index = 0;
    for(std::vector <complex_double>::iterator itr = tempPoles.begin(); itr != tempPoles.end(); itr++){
        poles[index] = *itr;
        index++;
    }
    ...
    switch(filter){
        ...
        default: {
#if LOG_OUTPUT
            std::cout << "[Butterworth Filter Design] Unknown Filter Type" << std::endl;
#endif
            return false;
        }
    }
    ...
    for(uint32_t i = 0; i < numPoles; i++){
        if(poles[i].real() > 0){
#if LOG_OUTPUT
            std::cerr << "[Butterworth Filter Design] Error: poles must be in the left half plane" << std::endl;
#endif
            return false;
        }
    }
    ...
    if(!s2Z()){
#if LOG_OUTPUT
        std::cerr << "[Butterworth Filter Design] Error: s2Z failed" << std::endl << std::endl;
#endif
        return false;
    }
    ...
    if(!zp2SOS()){
#if LOG_OUTPUT
        std::cerr << "[Butterworth Filter Design] Error: zp2SOS failed" << std::endl << std::endl;
#endif
        return false;
    }
    ...
    std::cout << std::endl;
    std::cout << "[Butterworth Filter Design] preBLTgain = " << preBLTgain << std::endl;
    std::cout << "[Butterworth Filter Design] gain = " << gain << std::endl;

    std::cout << "[Butterworth Filter Design] ba[0] = " << ba[0] << std::endl;
    std::cout << "[Butterworth Filter Design] coeff size = " << nba << std::endl << std::endl;

    for(uint32_t i = 0; i < (nba - 1) / 4; i++){
        // b0,b1,b2: a0,a1,a2:= 1.0, ba[4*i+1], ba[4*i+2], 1.0, ba[4*i+3], ba[4*i+4]
        std::cout << "[Butterworth Filter Design] Biquads:= 1.0 " << ba[4 * i + 1] << " "
        << ba[4 * i + 2] << " "
        << ba[4 * i + 3] << " "
        << ba[4 * i + 4] << std::endl;
    }
...
bool Butterworth::coefficientsEQ(BUTTERWORTH_TYPE filter, double fs, double f1,
...
void Butterworth::convert2bandstop(){
    ...
    int index = 0;
    for(std::vector <complex_double>::iterator itr = ztmp.begin(); itr != ztmp.end(); itr++){
        zeros[index] = *itr;
        index++;
    }
